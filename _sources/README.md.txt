...
[`moptipy`](./_static/moptipy_flyer.pdf) is a library with implementations of metaheuristic optimization methods in Python&nbsp;3.10 that also offers an environment for replicable experiments.
The framework, [algorithm implementations](#implemented-algorithms), and the library design are accompanied by the book [Optimization Algorithms](https://thomasweise.github.io/oa/).
...
`moptipy` provides an [API](./moptipy.api.html), several [algorithm implementations](./moptipy.algorithms.html#module-moptipy.algorithms), as well as [experiment execution](./moptipy.api.html#module-moptipy.api.experiment) and [evaluation](./moptipy.evaluation.html#module-moptipy.evaluation) facilities for metaheuristics.

A metaheuristic algorithm can be a black-box method, which can solve problems without deeper knowledge about their nature.
Such a black-box algorithm only requires methods to [create](./moptipy.api.html#moptipy.api.operators.Op0) and [modify](./moptipy.api.html#moptipy.api.operators.Op1) points in the [search space](./moptipy.api.html#module-moptipy.api.space) and to [evaluate their quality](./moptipy.api.html#module-moptipy.api.objective).
...
They allow us to plug in almost arbitrary [search operators](./moptipy.api.html#module-moptipy.api.operators), [search spaces](./moptipy.api.html#module-moptipy.api.space), and [objective functions](./moptipy.api.html#module-moptipy.api.objective).
...
Or one could design an algorithm for a specific search space, say, the [`n`-dimensional real numbers](./moptipy.algorithms.so.vector.html), which could then make use of the special features of this space, such as arithmetic and geometric relationships of the points within it.
Or one could design an algorithm for a specific problem, making use of specific features of the objective function.
Finally, there are [multi-objective optimization problems](./moptipy.api.html#module-moptipy.api.mo_problem) where multiple, potentially conflicting, criteria need to be optimized at once.

Within our [`moptipy`](./_static/moptipy_flyer.pdf) framework, you can implement algorithms of all of these types under a unified [API](./moptipy.api.html).
Our package already provides a growing set of [algorithms](#implemented-algorithms) and adaptations to different [search spaces](#implemented-search-spaces-and-operators) as well as a set of well-known [optimization problems](#implemented-problems).
What `moptipy` *also* offers is an [experiment execution facility](./moptipy.api.html#module-moptipy.api.experiment) that can collect detailed [log information](#data-formats) and [evaluate](#evaluating-experiments) the gathered results in a [*reproducible* fashion](#reproducibility).
The `moptipy` API supports both single-objective and multi-objective optimization.
A set of "[How-Tos](#how-tos)" is given in [Section 3](#how-tos) and a longer [list of examples](#examples) is given in [Section 7](#examples).
You can also take a look at our [`moptipy` flyer](./_static/moptipy_flyer.pdf).
Examples and practical applications of `moptipy` can be found in the [`moptipyapps`](./apps/) package, which is available on [GitHub](https://github.com/thomasWeise/moptipyapps) and in [`PyPi`](https://pypi.org/project/moptipyapps/).
...
All dependencies for using and running `moptipy` are listed at [here](./requirements_txt.html).
...
Here, we talk mainly about directly applying one or multiple [optimization algorithm(s)](#implemented-algorithms) to one or multiple [optimization problem](#implemented-problems) instance(s).
In [Section 5 on Data Formats](#data-formats), we give examples and specifications of the log files that our system produces and how you can export the data to other formats.
Later, in [Section 6 on Evaluating Experiments](#evaluating-experiments), we provide several examples on how to evaluate and visualize the results of experiments.
In [Section 7 on examples](#examples), we list all the examples that ship with moptipy.


## 3.1. How to Apply 1 Optimization Algorithm Once to 1 Problem Instance

The most basic task that we can do in the domain of optimization is to apply one [algorithm](#implemented-algorithms) to one instance of an [optimization problem](#implemented-problems).
In our framework, we refer to this as an "execution."
You can prepare an execution using the class [`Execution`](./moptipy.api.html#moptipy.api.execution.Execution) in the module [moptipy.api.execution](./moptipy.api.html#module-moptipy.api.execution).
...
In our case, the class [`Execution`](./moptipy.api.html#moptipy.api.execution.Execution) allows you to compose all the elements necessary for the algorithm run and then it performs it and provides you the end results of that execution.

So first, you create an instance `ex` of [`Execution`](./moptipy.api.html#moptipy.api.execution.Execution).
Then you set the [algorithm](./moptipy.api.html#moptipy.api.algorithm.Algorithm) that should be applied via the method [`ex.set_algorithm(...)`](./moptipy.api.html#moptipy.api.execution.Execution.set_algorithm).
Then you set the [objective function](./moptipy.api.html#moptipy.api.objective.Objective) via the method [`ex.set_objective(...)`](./moptipy.api.html#moptipy.api.execution.Execution.set_objective).

Then, via [`ex.set_solution_space(...)`](./moptipy.api.html#moptipy.api.execution.Execution.set_solution_space) you set the solution [space](./moptipy.api.html#moptipy.api.space.Space) that contains all possible solutions and is explored by the algorithm.
The solution space is an instance of the class [`Space`](./moptipy.api.html#moptipy.api.space.Space).
...
It is used internally by the `moptipy` system to automatically maintain copies of the current best solution, to check if the solutions are indeed valid once the algorithm finishes, and to convert the solution to a string to store it in the [log files](#log-files).

If the search and solution spaces are different, then you can also set a search [space](./moptipy.api.html#moptipy.api.space.Space) via [`ex.set_search_space(...)`](./moptipy.api.html#moptipy.api.execution.Execution.set_search_space) and an [encoding](./moptipy.api.html#module-moptipy.api.encoding) via [`ex.set_encoding(...)`](./moptipy.api.html#moptipy.api.execution.Execution.set_encoding).
This is not necessary if the algorithm works directly on the solutions (as in our example below).

Each application of an [optimization algorithm](./moptipy.api.html#module-moptipy.api.algorithm) to a problem instance will also be provided with a [random number generator](./moptipy.api.html#moptipy.api.process.Process.get_random) and it *must* only use this random number generator for randomization and no other sources of randomness.
You can set the seed for this random number generator via [`ex.set_rand_seed(...)`](./moptipy.api.html#moptipy.api.execution.Execution.set_rand_seed).
...
Furthermore, you can also set the maximum number of candidate solutions that the optimization algorithm is allowed to investigate via [`ex.set_max_fes(...)`](./moptipy.api.html#moptipy.api.execution.Execution.set_max_fes), the maximum runtime budget in milliseconds via [`ex.set_max_time_millis(...)`](./moptipy.api.html#moptipy.api.execution.Execution.set_max_time_millis), and a goal objective value via [`ex.set_goal_f(...)`](./moptipy.api.html#moptipy.api.execution.Execution.set_goal_f) (the algorithm should stop after reaching it).
Notice that optimization algorithms may not terminate unless the system tells them to, i.e., unless [`process.should_terminate()`](./moptipy.api.html#moptipy.api.process.Process.should_terminate) returns `True`, which is triggered by the termination conditions you define as stated above.
...
Finally, you can also set the path to a log file via [`ex.set_log_file(...)`](./moptipy.api.html#moptipy.api.execution.Execution.set_log_file).
If you specify a log file, the system will automatically gather system information and collect the end result.
Via [`ex.set_log_improvements(True)`](./moptipy.api.html#moptipy.api.execution.Execution.set_log_improvements), you can instruct the system to also collect the progress of the algorithm in terms of improving moves by default.
In the rare case that you want to log every single move that the algorithm makes, you could call [`ex.set_log_all_fes(True)`](./moptipy.api.html#moptipy.api.execution.Execution.set_log_all_fes).
All the collected data will be stored in a [text file](#log-files) *after* the algorithm has completed and you have left the process scope (see below).

<a href="./_static/process_architecture.svg">
<img alt="The process architecture of the moptipy framework." src="./_static/process_architecture.svg" style="width:50%;max-width:60%;min-width:30%;float:left;padding-right:2em;padding-top:0.5em;padding-bottom:0.5em" />
</a>

Anyway, after you have completed building the execution, you can run the process you have configured via `ex.execute()`.
This method returns an instance of [`Process`](./moptipy.api.html#module-moptipy.api.process).
...
Let us now look at a concrete example, which is also available as file [examples/single_run_rls_onemax](./examples/single_run_rls_onemax_py.html).
As example domain, we use [bit strings](./moptipy.spaces.html#module-moptipy.spaces.bitstrings) of length `n = 10` and try to solve the well-known [`OneMax`](./moptipy.examples.bitstrings.html#module-moptipy.examples.bitstrings.onemax) problem using the well-known [`RLS`](./moptipy.algorithms.so.html#module-moptipy.algorithms.so.rls).
...
You can also compare this output to the [example for log files](#example-for-single-objective-optimization) further down this text.
...
It could be a string with its identifying name, it could be the [objective function](./moptipy.api.html#moptipy.api.objective.Objective) itself, or a data structure with the instance data (as is the case for the Job Shop Scheduling Problem used in our book, where we use the class [Instance](./moptipy.examples.jssp.html#module-moptipy.examples.jssp.instance)).
The important thing is that the `__str__` method of the instance object will return a short string that can be used in file names of [log files](#log-files).

The second concept to understand here are "setups."
A "setup" is basically an almost fully configured [`Execution`](./moptipy.api.html#module-moptipy.api.execution) (see the [previous section](#how-to-apply-1-optimization-algorithm-once-to-1-problem-instance) for a detailed discussion of Executions.)
...
Additionally, you provide a sequence of callables (functions or lambdas), each of which receiving one "instance" as input and should return an almost fully configured [`Execution`](./moptipy.api.html#module-moptipy.api.execution).
...
`moptipy` also supports [parallel and distributed experiments](#parallel-and-distributed-experiments).
...
This is discussed [here](#parallel-and-distributed-experiments).

All of this is passed to the function [`run_experiment`](./moptipy.api.html#moptipy.api.experiment.run_experiment) in module [`moptipy.api.experiment`](./moptipy.api.html#module-moptipy.api.experiment).

This function will do all the work and generate a [folder structure](#file-names-and-folder-structure) of log files.
It will spawn the right number of processes, use your functions to generate "instances" and "setups," and execute them.
It will also [automatically determine](./moptipy.utils.html#moptipy.utils.nputils.rand_seeds_from_str) the random seed for each run.
The seed sequence is determined from the instance name using a [deterministic procedure](./moptipy.utils.html#moptipy.utils.nputils.rand_seeds_from_str) and therefore [reproducible](#reproducibility).
...
We use again the  [bit strings domain](./moptipy.spaces.html#module-moptipy.spaces.bitstrings).
We explore two problems ([`OneMax`](./moptipy.examples.bitstrings.html#module-moptipy.examples.bitstrings.onemax) and [`LeadingOnes`](./moptipy.examples.bitstrings.html#module-moptipy.examples.bitstrings.leadingones)) of two different sizes each, leading to four problem instances in total.
We apply the well-known [`RLS`](./moptipy.algorithms.so.html#module-moptipy.algorithms.so.rls) as well as the trivial [random sampling](./moptipy.algorithms.html#module-moptipy.algorithms.random_sampling).

The code below is available as file [examples/experiment_2_algorithms_4_problems](./examples/experiment_2_algorithms_4_problems_py.html).
Besides executing the experiment, it also prints the end results obtained from parsing the log files (see [Section 5.2.](#end-result-csv-files) for more information).
...
When you invoke [`run_experiment`](./moptipy.api.html#moptipy.api.experiment.run_experiment)  and not specify `n_threads = 1`, the system will automatically determine a suitable number of processes to launch in order to execute the runs of the experiment in parallel.
...
The trick is that we create the random seeds in a deterministic fashion so that each experiment on each node will have the same seeds and, hence, the [same names for the log files](#file-names-and-folder-structure).
...
You may need more components, but if you have these three, then you can [run an experiment](#how-to-run-a-series-of-experiments).


### 3.3.1. Define a New Problem Type

At the core of all optimization problems lies the objective function.
All objective functions in [`moptipy`](./) are instances of the class [Objective](./moptipy.api.html#moptipy.api.objective.Objective).
...
- [`evaluate(x)`](./moptipy.api.html#moptipy.api.objective.Objective.evaluate) receives a candidate solution `x` as input and must return either an `int` or a `float` rating its quality (smaller values are *better*) and
...
- [`lower_bound()`](./moptipy.api.html#moptipy.api.objective.Objective.lower_bound) returns either an `int` or a `float` with the lower bound of the objective value.
...
- [`upper_bound()`](./moptipy.api.html#moptipy.api.objective.Objective.upper_bound) returns either an `int` or a `float` with the upper bound of the objective value.
...
Basically, we want that our optimization algorithm works on [permutations](./moptipy.spaces.html#module-moptipy.spaces.permutations) of `n` numbers and is searching for the sorted permutation.
...
We provide the corresponding code in [Section 3.3.3](#applying-an-own-algorithm-to-an-own-problem) below.


### 3.3.2. Define a New Algorithm

While [`moptipy`](./) comes with several [well-known algorithms](#implemented-algorithms) out-of-the-box, you can of course also implement your own [algorithms](./moptipy.api.html#moptipy.api.algorithm.Algorithm).
These can then make use of the existing [spaces](./moptipy.api.html#moptipy.api.space.Space) and [search operators](./moptipy.api.html#module-moptipy.api.operators) &ndash; or not.
Let us here create an example algorithm implementation that does *not* use any of the pre-defined [search operators](./moptipy.api.html#module-moptipy.api.operators).

All optimization algorithms must be subclasses of the class [Algorithm](./moptipy.api.html#moptipy.api.algorithm.Algorithm).
Each of them must implement two methods, as described in the [documentation](./moptipy.api.html#module-moptipy.api.algorithm):

- [`solve(process)`](./moptipy.api.html#moptipy.api.algorithm.Algorithm.solve) receives an instance of [`Process`](./moptipy.api.html#module-moptipy.api.process), which provides the operations to work with the search space, to evaluate solutions, the termination criterion, and the random number generator.
...
- [`initialize()`](./moptipy.api.html#moptipy.api.component.Component.initialize) initializes all sub-components of the algorithms and is called *before* each run.
  The base class [`Component`](./moptipy.api.html#moptipy.api.component.Component), from which all elements of the optimization API are derived, already has this method.
  If a new algorithm uses, for example, a [selection](./moptipy.algorithms.modules.html#module-moptipy.algorithms.modules.selection) algorithm, a [temperature schedule](./moptipy.algorithms.modules.html#module-moptipy.algorithms.modules.selection), or a [search operator](./moptipy.api.html#module-moptipy.api.operators), it needs to invoke the `initialize()` methods of these components from its own `initialize()` method.
- [`log_parameters_to(...)`](./moptipy.api.html#moptipy.api.component.Component.log_parameters_to) is used to store all the configuration parameters of an algorithm to a [log section](./moptipy.utils.html#moptipy.utils.logger.KeyValueLogSection).
  If the algorithm has any sub-components, it must here invoke the `log_parameters_to(...)` method of these components.
  In this case, it can pass different [prefix scopes](./moptipy.utils.html#moptipy.utils.logger.KeyValueLogSection.scope) to the `log_parameters_to(...)` methods of its components.
  Then, different prefixes can be added to each component's parameter keys, ensuring that all keys are unique.

The instance `process` of [`Process`](./moptipy.api.html#module-moptipy.api.process) passed to the function [`solve`](./moptipy.api.html#moptipy.api.algorithm.Algorithm.solve) is a key element of our [`moptipy`](./) API.
If the algorithm needs a data structure to hold a point in the search space, it should invoke [`process.create()`](./moptipy.api.html#moptipy.api.space.Space.create).
If it needs to copy the point `source` to the point `dest`, it should invoke [`process.copy(dest, source)`](./moptipy.api.html#moptipy.api.space.Space.copy).

If it wants to know the quality of the point `x`, it should invoke [`process.evaluate(x)`](./moptipy.api.html#moptipy.api.process.Process.evaluate).
This function will forward the call to the actual objective function (see, e.g., [Section 3.3.1](#define-a-new-problem-type) above).
...
Before every single call to [`process.evaluate()`](./moptipy.api.html#moptipy.api.process.Process.evaluate), you should invoke [`process.should_terminate()`](./moptipy.api.html#moptipy.api.process.Process.should_terminate).
...
Since many optimization algorithms make random choices, the function [`process.get_random()`](./moptipy.api.html#moptipy.api.process.Process.get_random) returns a [random number generator](https://numpy.org/doc/stable/reference/random/generator.html).
...
The `process` also can provide information about the best-so-far [solution](./moptipy.api.html#moptipy.api.process.Process.get_copy_of_best_y) or [point in the search space](./moptipy.api.html#moptipy.api.process.Process.get_copy_of_best_x), the consumed [runtime](./moptipy.api.html#moptipy.api.process.Process.get_consumed_time_millis) and [FEs](./moptipy.api.html#moptipy.api.process.Process.get_consumed_fes), as well as [when the last improvement was achieved](./moptipy.api.html#moptipy.api.process.Process.get_last_improvement_fe).
...
The search space that we use are the [permutations](./moptipy.spaces.html#module-moptipy.spaces.permutations) of `n` numbers.
...
We repeat this until [`process.should_terminate()`](./moptipy.api.html#moptipy.api.process.Process.should_terminate) becomes `True`.
All of this is implemented in the source code example below in [Section 3.3.3](#applying-an-own-algorithm-to-an-own-problem).

Finally, as a side note:
Our system can automatically store the results of optimization processes in [log file](#log-files).
The `process` API also allows your algorithm to store additional information in these files:
First, you can check with [`process.has_log()`](./moptipy.api.html#moptipy.api.process.Process.has_log) if the process was configured to store information in a log file.
If this function returns `True`, then you can create a [section](#log-file-sections) with a given `title` in the log files that should contain one single string `text` by calling  [`process.add_log_section(title, text)`](./moptipy.api.html#moptipy.api.process.Process.add_log_section).
Make sure that all section `title`s are unique.
All such sections will be appended at the end of the log files, wrapped in `BEGIN_title` and `END_title` markers, as prescribed by [our log file format](#log-file-sections).


### 3.3.3. Applying an Own Algorithm to an Own Problem

The following code combines our [own algorithm](#define-a-new-algorithm) and our [own problem type](#define-a-new-problem-type) that we discussed in the prior two sections and executes an experiment.
It is available as file [examples/experiment_own_algorithm_and_problem](./examples/experiment_own_algorithm_and_problem_py.html).
Notice how we provide functions for generating both the problem instances (here the objective functions) and the algorithm setups exactly as we described in [Section 3.2.](#how-to-run-a-series-of-experiments) above.
...
Here we list the [algorithms](#implemented-algorithms), [search spaces](#implemented-search-spaces-and-operators), and [optimization problems](#implemented-problems) that we implement in our [`moptipy`](./) framework.
...
- [Single Random Sample](./moptipy.algorithms.html#module-moptipy.algorithms.single_random_sample) creates and evaluates exactly one single random solution.
- [Random Sampling](./moptipy.algorithms.html#module-moptipy.algorithms.random_sampling) keeps creating random solutions until the computational budget is exhausted.
- [Random Walk](./moptipy.algorithms.html#module-moptipy.algorithms.random_sampling) creates a random solution and then keeps applying the unary search operator and always accepts the result.


### 4.1.1. Single-Objective Optimization

Here we list optimization algorithms that optimize a *single* [objective function](./moptipy.api.html#moptipy.api.objective.Objective).


#### 4.1.1.1. Single-Objective Optimization with Arbitrary Search Spaces

The first set of algorithms is general, i.e., can work with arbitrary [search spaces](#implemented-search-spaces-and-operators).

- The simple [Hill Climber](./moptipy.algorithms.so.html#module-moptipy.algorithms.so.hill_climber) creates a random solution as initial best-so-far solution and then iteratively applies the unary search operator to the best-so-far solution.
  When the result of the unary operator is better, it becomes the new best-so-far solution, otherwise it is discarded.
- The [Hill Climber with Restarts](./moptipy.algorithms.so.html#module-moptipy.algorithms.so.hill_climber_with_restarts) works exactly like the hill climber, but restarts at a new random solution after a fixed number of unsuccessful moves.
- A Random Local Search ([RLS](./moptipy.algorithms.so.html#module-moptipy.algorithms.so.rls)) also known as (1+1)&nbsp;EA works like the [Hill Climber](./moptipy.algorithms.so.html#module-moptipy.algorithms.so.hill_climber) as well, but accepts a new solution if it is *not worse* than the best-so-far solution (instead of requiring it to be strictly *better*, as the hill climber does).
- The [(&#x3BC;+&#x3BB;)&nbsp;EA](./moptipy.algorithms.so.html#module-moptipy.algorithms.so.ea), where "EA" stands for "Evolutionary Algorithm," is a simple population-based metaheuristic that starts with a population of `mu` random solutions.
  ...
  The `(1+1) EA` with `br=0` probability to use the binary operator is equivalent to [RLS](./moptipy.algorithms.so.html#module-moptipy.algorithms.so.rls).
- The [general EA](./moptipy.algorithms.so.html#module-moptipy.algorithms.so.general_ea) is a generalized version of the (&#x3BC;+&#x3BB;)&nbsp;EA that can additionally be configured with a [fitness assignment process](./moptipy.algorithms.so.fitnesses.html) and both survival and mating [selection algorithms](./moptipy.algorithms.modules.html#module-moptipy.algorithms.modules.selection).
- Simulated Annealing ([SA](./moptipy.algorithms.so.html#module-moptipy.algorithms.so.simulated_annealing)) is similar to [RLS](./moptipy.algorithms.so.html#module-moptipy.algorithms.so.rls) but sometimes accepts worsening moves.
  The probability to accept such moves gets smaller the worse the moves are and the longer the search continues.
- The (&#x3BC;+&#x3BB;)&nbsp;Memetic Algorithm ([MA](./moptipy.algorithms.so.html#module-moptipy.algorithms.so.ma)) works like the [(&#x3BC;+&#x3BB;)&nbsp;EA](./moptipy.algorithms.so.html#module-moptipy.algorithms.so.ea) but it applies the binary search operator at a rate of&nbsp;100%.
  (In other words, it never uses the unary search operator).
  It refines the results of the nullary and binary search operators by using them as starting points of another algorithm, say an [SA](./moptipy.algorithms.so.html#module-moptipy.algorithms.so.simulated_annealing) or [RLS](./moptipy.algorithms.so.html#module-moptipy.algorithms.so.rls), which is executed for a pre-defined number of steps.
- The above [MA](./moptipy.algorithms.so.html#module-moptipy.algorithms.so.ma) can be configured to use [RLS](./moptipy.algorithms.so.html#module-moptipy.algorithms.so.rls) as local search.
  This special case is also implemented as hard-coded [MA-RLS](./moptipy.algorithms.so.html#module-moptipy.algorithms.so.marls), which should be a little bit more speed-efficient (due to hard-coding the local search instead of plugging it in), but otherwise it takes the exactly same route through the search space.
- The [general MA](./moptipy.algorithms.so.html#module-moptipy.algorithms.so.general_ma) is a generalized version of the (&#x3BC;+&#x3BB;)&nbsp;MA that can additionally be configured with a [fitness assignment process](./moptipy.algorithms.so.fitnesses.html) and both survival and mating [selection algorithms](./moptipy.algorithms.modules.html#module-moptipy.algorithms.modules.selection).
- The Plant Propagation Algorithm ([PPA](./moptipy.algorithms.so.html#module-moptipy.algorithms.so.ppa)) is a population-based metaheuristic that allocates both the number of new solutions to be derived from an existing solution via the unary search operator as well as the step size to be used by this operator based on the normalized objective value of the solution.


#### 4.1.1.2. Single-Objective Optimization with Continuous Search Space

The algorithms listed here are intended for single-objective optimization of [continuous search spaces](./moptipy.spaces.html#moptipy.spaces.vectorspace.VectorSpace).
They *only* work with search spaces that are instances of [`VectorSpace`](./moptipy.spaces.html#moptipy.spaces.vectorspace.VectorSpace).
Such spaces are defined by box-constraints over the *n*-dimensional real numbers.

- The quasi-Newton method by Broyden, Fletcher, Goldfarb, and Shanno ([BFGS](./moptipy.algorithms.so.vector.html#moptipy.algorithms.so.vector.scipy.BGFS)), wrapped from [SciPy](https://docs.scipy.org/doc/scipy/reference/optimize.minimize-bfgs.html).
- The Bound Optimization BY Quadratic Approximation algorithm ([BOBYQA](./moptipy.algorithms.so.vector.html#module-moptipy.algorithms.so.vector.pdfo)) from the library "Powell's Derivative-Free Optimization solvers" ([pdfo](https://www.pdfo.net)).
- The Conjugate Gradient ([CG](./moptipy.algorithms.so.vector.html#moptipy.algorithms.so.vector.scipy.CG)) algorithm, wrapped from [SciPy](https://docs.scipy.org/doc/scipy/reference/optimize.minimize-cg.html).
- The Covariance Matrix Adaptation Evolution Strategies [CMA-ES](./moptipy.algorithms.so.vector.html#moptipy.algorithms.so.vector.cmaes_lib.CMAES), [Separable CMA-ES](./moptipy.algorithms.so.vector.html#moptipy.algorithms.so.vector.cmaes_lib.SepCMAES), and [BIPOP-CMA-ES](./moptipy.algorithms.so.vector.html#moptipy.algorithms.so.vector.cmaes_lib.BiPopCMAES) from the library [cmaes](https://pypi.org/project/cmaes/).
- Differential Evolution ([DE](./moptipy.algorithms.so.vector.html#moptipy.algorithms.so.vector.scipy.DE)), wrapped from [SciPy](https://docs.scipy.org/doc/scipy/reference/generated/scipy.optimize.differential_evolution.html).
- The Downhill Simplex method based on the [Nelder-Mead](./moptipy.algorithms.so.vector.html#moptipy.algorithms.so.vector.scipy.NelderMead), wrapped from [SciPy](https://docs.scipy.org/doc/scipy/reference/optimize.minimize-neldermead.html).
- [Powell's Algorithm](./moptipy.algorithms.so.vector.html#moptipy.algorithms.so.vector.scipy.Powell), wrapped from [SciPy](https://docs.scipy.org/doc/scipy/reference/optimize.minimize-powell.html).
- The Sequential Least Squares Programming ([SLSQP](./moptipy.algorithms.so.vector.html#moptipy.algorithms.so.vector.scipy.SLSQP) algorithm, wrapped from [SciPy](https://docs.scipy.org/doc/scipy/reference/optimize.minimize-slsqp.html)).
- The Truncated Newton Method ([TNC](./moptipy.algorithms.so.vector.html#moptipy.algorithms.so.vector.scipy.TNC)), wrapped from [SciPy](https://docs.scipy.org/doc/scipy/reference/optimize.minimize-tnc.html).


### 4.1.2. Multi-Objective Optimization

The algorithms listed here are suitable for [multi-objective optimization](./moptipy.api.html#moptipy.api.mo_problem.MOProblem).
In other words, they try to minimize multiple [objective functions](./moptipy.api.html#moptipy.api.objective.Objective) at once.

- [Multi-Objective Random Local Search](./moptipy.algorithms.mo.html#moptipy.algorithms.mo.morls.MORLS) (MORLS) works exactly as [RLS](./moptipy.algorithms.so.html#moptipy.algorithms.so.rls.RLS), but it accepts a solution if it is not dominated by the current solution.
 This is *not* a good algorithm.
- The Fast Elitist Non-Dominated Sorting Genetic Algorithm ([NSGA-II](./moptipy.algorithms.mo.html#module-moptipy.algorithms.mo.nsga2)) is maybe the most popular multi-objective evolutionary algorithm.


## 4.2. Implemented Search Spaces and Operators

- [Bit Strings](./moptipy.spaces.html#moptipy.spaces.bitstrings.BitStrings) of a fixed length `n` are represented as `numpy` arrays as well.
  Here, each value can either be `True` or `False`.
  - Nullary Operators:
    - [random initialization](./moptipy.operators.bitstrings.html#moptipy.operators.bitstrings.op0_random.Op0Random) fills the string with random bits
  - Unary Operators:
    - [flip 1](./moptipy.operators.bitstrings.html#moptipy.operators.bitstrings.op1_flip1.Op1Flip1) flips one single bit.
    - [flip m/n](./moptipy.operators.bitstrings.html#moptipy.operators.bitstrings.op1_m_over_n_flip.Op1MoverNflip.op1) flips each bit independently with probability `m/n`.
    - [flip m](./moptipy.operators.bitstrings.html#module-moptipy.operators.bitstrings.op1_flip_m) is an operator with step width where the exact number of bits to flip can be determined.
  - Binary Operators:
    - [uniform crossover](./moptipy.operators.bitstrings.html#moptipy.operators.bitstrings.op2_uniform.Op2Uniform) randomly chooses, for each bit, from which of the two source strings it will be copied.
- [Permutations](./moptipy.spaces.html#module-moptipy.spaces.permutations) with and without Repetitions are represented as numpy arrays of integers.
  ...
  - Nullary Operators:
    - [Fisher-Yates shuffle](./moptipy.operators.permutations.html#moptipy.operators.permutations.op0_shuffle.Op0Shuffle) creates uniformly randomly distributed permutations.
  - Unary Operators:
    - [insert_1](./moptipy.operators.permutations.html#module-moptipy.operators.permutations.op1_insert1) removes one element from a permutation and inserts it elsewhere.
    - [swap 2](./moptipy.operators.permutations.html#moptipy.operators.permutations.op1_swap2.Op1Swap2) swaps exactly two (different) values.
    - [swap n](./moptipy.operators.permutations.html#moptipy.operators.permutations.op1_swapn.Op1SwapN) performs a random number of swaps.
    - [swap_exactly_n](./moptipy.operators.permutations.html#module-moptipy.operators.permutations.op1_swap_exactly_n) is a unary operator with step size that will change/swap exactly a given number of elements for permutations where each element occurs once and which will try to swap that many in permutations with repetitions (where it might not be possible to swap exactly the required number of elements).
    - [swap_try_n](./moptipy.operators.permutations.html#module-moptipy.operators.permutations.op1_swap_try_n) is a unary operator very similar to the `swap_exactly_n` operator, but it invests much less effort to achieve the number of prescribed swaps and thus is both much faster but also more likely to perform less swaps.
  - Binary Operators:
    - [generalized alternating position crossover](./moptipy.operators.permutations.html#moptipy.operators.permutations.op2_gap.Op2GeneralizedAlternatingPosition) chooses, for each index, from which of the two source permutations the (next not-yet-used) value should be copied
    - [order-based crossover](./moptipy.operators.permutations.html#moptipy.operators.permutations.op2_ox2.Op2OrderBased) randomly selects a set of indices and copies the elements from first source permutation to the same indices in the destination string. It then copies the remaining elements from the second source, maintaining the order in which they appear in the second source string.
- [Signed Permutations](./moptipy.spaces.html#module-moptipy.spaces.signed_permutations) with and without Repetitions are represented as numpy arrays of integers.
   They cannot contain the value `0`. All other values may occur either positive or negative, e.g., `5` or `-5`.
  - Nullary Operators:
    - [shuffle-and-flip](./moptipy.operators.signed_permutations.html#module-moptipy.operators.signed_permutations.op0_shuffle_and_flip) creates uniformly randomly distributed signed permutations.
  - Unary Operators:
    - [swap 2 or flip](./moptipy.operators.signed_permutations.html#module-moptipy.operators.signed_permutations.op1_swap_2_or_flip) either swaps exactly two (different) values or flips the sign of one value.
- [Ordered Choices](./moptipy.spaces.html#module-moptipy.spaces.ordered_choices) are a hybrid of permutations and combinations.
  ...
  - Nullary Operators:
    - [choose_and_shuffle](./moptipy.operators.ordered_choices.html#module-moptipy.operators.ordered_choices) picks one random value from each choice and shuffles them.
- [`n`-dimensional spaces of real numbers](./moptipy.spaces.html#moptipy.spaces.vectorspace.VectorSpace) are subsets of the `n`-dimensional real numbers.
  They are `numpy` arrays representing vectors of length `n`.
  On each dimension, a [lower](./moptipy.spaces.html#moptipy.spaces.vectorspace.VectorSpace.lower_bound) and an [upper bound](./moptipy.spaces.html#moptipy.spaces.vectorspace.VectorSpace.upper_bound) are imposed.
  - Nullary Operators:
    - [uniform sampling](./moptipy.operators.vectors.html#module-moptipy.operators.vectors.op0_uniform) samples a point from the uniform distribution resulting from the lower- and upper bound of the search space.
...
We do need a set of examples to [unit test](#unit-tests-and-static-analysis) these components, so a set of [example problems](./moptipy.examples.html#module-moptipy.examples) is indeed provided.
More examples and practical applications of `moptipy` can be found in the [`moptipyapps`](./apps/) package, which is available on [GitHub](https://github.com/thomasWeise/moptipyapps) and in [`PyPi`](https://pypi.org/project/moptipyapps/).
The `moptipyapps` package is where we will include future examples and benchmark cases, such as for the Quadratic Assignment Problem ([QAP](./apps/#the-quadratic-assignment-problem-qap)), the Traveling Salesperson Problem ([TSP](./apps/#the-traveling-salesperson-problem-tsp)), the Traveling Tournament Problem ([TTP](./apps/#the-traveling-tournament-problem-ttp)), Two-Dimensional [Bin Packing](./apps/#two-dimensional-bin-packing-problem), or [Dynamic Control](./apps/#dynamic-controller-synthesis).

- [Bit Strings](./moptipy.spaces.html#moptipy.spaces.bitstrings.BitStrings) of a fixed length `n`:
  - The minimization version of the [1D Ising Model](./moptipy.examples.bitstrings.html#moptipy.examples.bitstrings.ising1d.Ising1d), where the goal is to ensure that all bits have the same values as their neighbors.
  - The minimization version of the [Jump](./moptipy.examples.bitstrings.html#module-moptipy.examples.bitstrings.jump) problem, which is equivalent to OneMax, but has a deceptive region right before the optimum.
  - The minimization version of the well-known [LeadingOnes](./moptipy.examples.bitstrings.html#module-moptipy.examples.bitstrings.leadingones) problem, where the goal is to maximize the length of the trailing substring of all `True` bits.
  - The minimization version of the well-known [OneMax](./moptipy.examples.bitstrings.html#module-moptipy.examples.bitstrings.onemax) problem, where the goal is to maximize the number of `True` bits in a string.
  - The minimization version of the [Trap](./moptipy.examples.bitstrings.html#module-moptipy.examples.bitstrings.trap) problem, which is equivalent of OneMax, but with the optimum and worst-possible solution swapped.
    This problem is therefore highly deceptive.
  - The [W-Model](./moptipy.examples.bitstrings.html#module-moptipy.examples.bitstrings.w_model), a problem that exhibits tunable neutrality, epistasis, ruggedness, and deceptiveness.
  - The minimization version of the well-known [ZeroMax](./moptipy.examples.bitstrings.html#module-moptipy.examples.bitstrings.zeromax) problem, which is the exact opposite of OneMax and has the goal to find the bit string of all `False` values.
- [Permutations](./moptipy.spaces.html#moptipy.spaces.permutations.Permutations) (with and without Repetitions):
  - The NP-hard Job Shop Scheduling Problem ([JSSP](./moptipy.examples.jssp.html#module-moptipy.examples.jssp)), where the goal is to find an assignment of jobs to machines with the minimum makespan.
    On <https://thomasweise.github.io/oa_data/>, we provide several zip archives with results obtained with [`moptipy`](./) on the JSSP.
- [`n`-dimensional spaces of real numbers](./moptipy.spaces.html#moptipy.spaces.vectorspace.VectorSpace)
  - [Ackley's Function](./moptipy.examples.vectors.html#module-moptipy.examples.vectors.ackley)
  - [Sphere Function](./moptipy.examples.vectors.html#module-moptipy.examples.vectors.sphere)
...
For example, you could download the [results](https://thomasweise.github.io/oa_data/jssp/jssp_hcr_swapn.tar.xz) of the [hill climber with restarts](./moptipy.algorithms.so.html#module-moptipy.algorithms.so.hill_climber_with_restarts) on the Job Shop Scheduling Problem ([JSSP](./moptipy.examples.jssp.html#module-moptipy.examples.jssp)) using the [operator `swapn`](./moptipy.operators.permutations.html#module-moptipy.operators.permutations.op1_swapn) that swaps a randomly chosen number of (different) job IDs, for different restart settings.
...
2. Each log file contains all information needed to fully understand the algorithm run, such as
   1. The results in numerical form, e.g., the [best achieved objective value](#the-section-state).
   2. The result in textual form, e.g., the textual representation of the [best solution discovered](#the-result-sections)
      This allows us to later load, use, or validate the result.
   3. The [random seed](#the-section-setup) used.
   4. The [termination criteria](#the-section-setup) used, i.e., the maximum objective function evaluations or the maximum runtime or the goal objective value.
   5. The algorithm name and [configuration](#the-section-setup).
      This allows us to later understand what we did here and to reproduce the algorithm setup.
   6. The problem instance name and [parameters](#the-section-setup).
      This makes sure that we know which problem instance did we solve.
   7. The [system configuration](#the-section-sys_info), such as the CPU nd operating system and Python version and [library versions](#the-section-sys_info).
   ...
   8. [Errors](#the-error-sections), if any occurred.
      We can guard against errors using [unit tests](#unit-tests-and-static-analysis), but it may still happen that a run of the optimization algorithm crashed.
   ...
   9. The [progress](#the-section-progress) that the algorithm made over time, if capturing this information [was demanded](./moptipy.api.html#moptipy.api.execution.Execution.set_log_improvements).
   10. The [contents of the archive](#the-archive_j_x-and-archive_j_y-sections) of non-dominated solutions, if we perform [multi-objective optimization](#example-log-file-for-multi-objective-optimization).
   11. The [objective values](#the-archive_qualities-section) of the solutions in the archive of non-dominated solutions, if we perform [multi-objective optimization](#example-log-file-for-multi-objective-optimization).
...
- *[Semicolon-separated values](./moptipy.utils.html#moptipy.utils.logger.CsvLogSection)* can hold a series of data values, where each row is divided into multiple values and the values are separated by `;`.
...
- *[Key-values](./moptipy.utils.html#moptipy.utils.logger.KeyValueLogSection)* sections represent, well, values for keys in form of a mapping compatible with [YAML](https://yaml.org/spec/1.2.2/#mapping).
  ...
  This allows representing complex data such as [configuration parameters](#the-section-setup) in a rather straight-forward, easy-to-parse canonical way.
...
- *[Raw text](./moptipy.utils.html#moptipy.utils.logger.TextLogSection)* sections contain text without a general or a priori structure, e.g., the string representation of the best solutions found.
  ...
  This is useful for, for instance, storing the [final solutions](#the-result-sections) of runs or [exceptions caught during the runs](#the-error-sections).
...
When setting up an algorithm execution, you can specify whether or not you want to [log the progress](./moptipy.api.html#moptipy.api.execution.Execution.set_log_improvements) of the algorithm.
...
You can also choose if you want to [log all algorithm steps](./moptipy.api.html#moptipy.api.execution.Execution.set_log_all_fes) or [only the improving moves](./moptipy.api.html#moptipy.api.execution.Execution.set_log_improvements), the latter being the default behavior.
...
Notice that even if you do not choose to log the algorithm's progress at all, the [section `STATE`](#the-section-state) with the objective value of the best solution encountered, the FE when it was found, and the consumed runtime, as well as the [`RESULT_*` sections](#the-result-sections) with the best encountered candidate solution and point in the search space, and also the [`SETUP`](#the-section-setup) and [`SYS_INFO`](#the-section-sys_info) still will be included in the log files.
...
`f` then stands for the [scalarized](./moptipy.api.html#moptipy.api.mo_problem.MOProblem.evaluate) version of the objective values.
...
- `bestF` then corresponds to the best [scalarization](./moptipy.api.html#moptipy.api.mo_problem.MOProblem.evaluate) result, i.e., the best value achieved by the scalarization of the objective value vector during the search,
...
- [process](./moptipy.api.html#moptipy.api.process.Process) wrapper parameters (scope `p`):
  ...
  - `p.goalF`: the goal objective value, if specified (or computed via the `lower_bound()` of the [objective function](./moptipy.api.html#moptipy.api.objective.Objective))
...
- [algorithm](./moptipy.api.html#moptipy.api.algorithm.Algorithm) parameters: scope `a`, includes algorithm `name`, `class`, etc.
- solution [space](./moptipy.api.html#moptipy.api.space.Space) scope `y`, includes `name` and `class` of solution space
- [objective function](./moptipy.api.html#moptipy.api.objective.Objective) information: scope `f`.
  If multi-objective optimization is performed, this is the scope of the [multi-objective problem](./moptipy.api.html#moptipy.api.mo_problem.MOProblem).
  There will be a sub-scope `f.fi` for the `i`th objective function (`i` starts at `0`).
- search [space](./moptipy.api.html#moptipy.api.space.Space) information (if search space is different from solution space): scope `x`
- [encoding](./moptipy.api.html#moptipy.api.encoding.Encoding) information (if encoding is defined): scope `g`
- [archive pruner](./moptipy.api.html#moptipy.api.mo_archive.MOArchivePruner) information (in case of multi-objective optimization): scope `ap`.

If you implement an own [algorithm](./moptipy.api.html#moptipy.api.algorithm.Algorithm), [objective function](./moptipy.api.html#moptipy.api.objective.Objective), [space](./moptipy.api.html#moptipy.api.space.Space), or your own [search operators](./moptipy.api.html#module-moptipy.api.operators), then you can overwrite the method [`log_parameters_to(logger)`](./moptipy.api.html#moptipy.api.component.Component.log_parameters_to).
...
This can be done by invoking the function [`add_dependency`](./moptipy.utils.html#moptipy.utils.sys_info.add_dependency) **before** running any experiment.
...
Both sections are plain texts, the results of the [`to_str`](./moptipy.api.html#moptipy.api.space.Space.to_str) method of the corresponding [`Space`](./moptipy.api.html#moptipy.api.space.Space) instances for the search and solution spaces.
...
The sections are plain texts, the results of the [`to_str`](./moptipy.api.html#moptipy.api.space.Space.to_str) method of the corresponding [`Space`](./moptipy.api.html#moptipy.api.space.Space) instances for the search and solution spaces.


### 5.1.3. Example for Single-Objective Optimization

You can execute the following Python code to obtain an example log file.
This code is also available in file [examples/log_file_jssp.py](./examples/log_file_jssp_py.html):
...
You can execute the following Python code to obtain an example log file for [multi-objective optimization](./moptipy.api.html#module-moptipy.api.mo_problem).
...
This code is also available in file [examples/mo_example_nsga2_bits.py](./examples/mo_example_nsga2_bits_py.html):
...
While a [log file](#log-files) contains all the data of a single run, you often want to get just the basic measurements, such as the result objective values, from all runs of one experiment in a single file.
The class [`moptipy.evaluation.end_results.EndResult`](./moptipy.evaluation.html#moptipy.evaluation.end_results.EndResult) provides the tools needed to parse all log files, extract these information, and store them into a semicolon-separated-values formatted file.
...
where `source_dir` should be the root directory with the experimental data (see [Section 5.1.1](#file-names-and-folder-structure))) and `dest_file` is the path to the CSV file to write.
...
1. `algorithm`: the [algorithm](./moptipy.api.html#module-moptipy.api.algorithm) that was executed
2. `instance`: the instance it was applied to
3. `objective`: the name of the [objective function](./moptipy.api.html#module-moptipy.api.objective)
4. optionally `encoding`: the name of the encoding, if any [encoding](./moptipy.api.html#module-moptipy.api.encoding) was used.
...
11. optionally `goalF`: the [goal objective value](./moptipy.api.html#moptipy.api.execution.Execution.set_goal_f), if specified, otherwise omitted.
...
12. optionally `maxFEs`: the computational budget in terms of the [maximum number of permitted FEs](./moptipy.api.html#moptipy.api.execution.Execution.set_max_fes), if specified, otherwise omitted.
...
13. optionally `maxTimeMillis`: the computational budget in terms of the [maximum runtime in milliseconds](./moptipy.api.html#moptipy.api.execution.Execution.set_max_time_millis), if specified, otherwise omitted.
...
We can do that with the code below, which is also available as file [examples/end_results_jssp.py](./examples/end_results_jssp_py.html).
...
The class [`moptipy.evaluation.end_statistics.EndStatistics`](./moptipy.evaluation.html#moptipy.evaluation.end_statistics.EndStatistics) provides the tools needed to aggregate statistics over sequences of [`moptipy.evaluation.end_results.EndResult`](./moptipy.evaluation.html#moptipy.evaluation.end_results.EndResult) and to store them into a semicolon-separated-values formatted file.
The files generated this way can easily be imported into applications like Microsoft Excel.
...
where `source` should either be the root directory with the experimental data (see [Section 5.1.1](#file-names-and-folder-structure))) or the path to a [end results CSV file](#end-result-csv-files) and `dest_file` is the path to the CSV file to write.
...
We can basically execute the same abridged experiment as in the [previous section](#an-example-for-end-results-files), but now take the aggregation of information one step further with the code below.
This code is also available as file [examples/end_statistics_jssp](./examples/end_statistics_jssp_py.html).
...
On one hand, you can [export](#exporting-data) the data to formats that can be processed by other tools.
On the other hand, you can plot a variety of different diagrams.
These diagrams can then be [stored](./moptipy.utils.html#module-moptipy.utils.plot_utils) in different formats, such as `svg` (for the web) or `pdf` (for scientific papers).
...
The [End Results CSV format](#end-result-csv-files) produces semicolon-separated-values files that include the states of each run.
...
This is, of course, only possible if we logged algorithm progress (i.e., did at least [`set_log_improvements(True)`](./moptipy.api.html#moptipy.api.execution.Execution.set_log_improvements)) over runtime and if these virtual limits are less or equal to the actual termination criteria.

The [End Results Statistics CSV format](#end-result-statistics-csv-files) allows you to export statistics aggregated, e.g., over the instance-algorithm combinations, for instance over all algorithms, or for one algorithm over all instances.
...
This can be done by the function [moptipy_to_ioh_analyzer](./moptipy.evaluation.html#module-moptipy.evaluation.ioh_analyzer), which accepts a source directory in the [`moptipy` structure](#file-names-and-folder-structure) and a path to a destination folder where the `IOHprofiler`-formatted data will be stored.
...
where `source_dir` should be the root directory with the experimental data (see [Section 5.1.1](#file-names-and-folder-structure))) and `dest_dir` is the directory where the IOHprofiler-formatted data should be written.


## 6.2. Progress Plots

In the file [examples/progress_plot.py](./examples/progress_plot_py.html), you can find some code running a small experiment and creating "progress plots."
...
<a href="./_static/progress_single_runs_and_mean_f_over_fes.png">
<img alt="Example for a progress plot combining statistics and single runs" src="./_static/progress_single_runs_and_mean_f_over_fes.png" style="width:70%;max-width:70%;min-width:70%" />
</a>

Progress plots are implemented in the module [moptipy.evaluation.plot_progress](./moptipy.evaluation.html#module-moptipy.evaluation.plot_progress).


## 6.3. End Results Plot

In the file [examples/end_results_plot.py](./examples/end_results_plot_py.html), you can find some code running a small experiment and creating "end results plots."
...
Then you get 50 [log files](#log-files) and each of them contains the best solution discovered by the corresponding run.
...
<a href="./_static/end_results_scaled.png">
<img alt="Example for an end result plot" src="./_static/end_results_scaled.png" style="width:70%;max-width:70%;min-width:70%" />
</a>

In the file [end_results_with_limits_plot.py](./examples/end_results_with_limits_plot_py.html), you can find an example of the interplay of this type of plots with the "virtual" runtime limits that can be specified when parsing [`moptipy.evaluation.end_results.EndResult`](./moptipy.evaluation.html#moptipy.evaluation.end_results.EndResult)s.
...
<a href="./_static/selected_end_results.png">
<img alt="Example for end results plots at different time limits" src="./_static/selected_end_results.png" style="width:70%;max-width:70%;min-width:70%" />
</a>

The end result plots are implemented in the module [moptipy.evaluation.plot_end_results](./moptipy.evaluation.html#module-moptipy.evaluation.plot_end_results).


## 6.4. ECDF Plots

In the file [examples/ecdf_plot.py](./examples/ecdf_plot_py.html), you can find some code running a small experiment and creating "ECDF plots."
...
<a href="./_static/ecdf_over_log_fes.png">
<img alt="Example for an ECDF plot combining statistics and single runs" src="./_static/ecdf_over_log_fes.png" style="width:70%;max-width:70%;min-width:70%" />
</a>

ECDF plots are implemented in the module [moptipy.evaluation.plot_ecdf](./moptipy.evaluation.html#module-moptipy.evaluation.plot_ecdf).


## 6.5. Expected Running Time (ERT) Plots

In the file [examples/ert_plot.py](./examples/ert_plot_py.html), you can find some code running a small experiment and creating empirically estimated Expected Running Time (ERT) plots.
...
<a href="./_static/log_ert_over_f.png">
<img alt="Example for an ERT plot of RLS and Random Walk on OneMax with 12 bits." src="./_static/log_ert_over_f.png" style="width:70%;max-width:70%;min-width:70%" />
</a>
...
The ERT plots are implemented in the module [moptipy.evaluation.plot_ert](./moptipy.evaluation.html#module-moptipy.evaluation.plot_ert).


## 6.6. ERT-ECDF Plots

In the file [examples/ertecdf_plot.py](./examples/ertecdf_plot_py.html), you can find some code running a small experiment and creating ERT-ECDF plots.
These plots combine the concepts of [ERTs](#expected-running-time-ert-plots) with [ECDFs](#ecdf-plots):
...
Like ECDF-plots, the ERT-ECDF plots are implemented in the module [moptipy.evaluation.plot_ecdf](./moptipy.evaluation.html#module-moptipy.evaluation.plot_ecdf).

<a href="./_static/ertecdf_over_log_fes.png">
<img alt="Example for an ERT-ECDF plot of a RLS on OneMax several OneMax instances." src="./_static/ertecdf_over_log_fes.png" style="width:70%;max-width:70%;min-width:70%" />
</a>
...
The function  [plot_end_statistics_over_param](./moptipy.evaluation.html#module-moptipy.evaluation.plot_end_statistics_over_parameter) can do both:

In [examples/end_statistics_over_feature_plot.py](./examples/end_statistics_over_feature_plot_py.html), it is used to visualize the [`ERT`](#expected-running-time-ert-plots) of a simple [RLS algorithm](./moptipy.algorithms.so.html#module-moptipy.algorithms.so.rls) over the instance size `n` of the [OneMax problem](./moptipy.examples.bitstrings.html#module-moptipy.examples.bitstrings.onemax).
...
We then load the end results and convert them to [end result statistics](#the-end-result-statistics-file-format).
All we need to tell our system how it can deduce the value of the feature from an [EndStatistics](./moptipy.evaluation.html#module-moptipy.evaluation.end_statistics) and which statistic we want to plot (here: `ertFEs`) and we are good:

<a href="./_static/ert_over_onemax_n.png">
<img alt="Example for the ERT of a RLS on OneMax plotted over the instance size n over several OneMax instances." src="./_static/ert_over_onemax_n.png" style="width:70%;max-width:70%;min-width:70%" />
</a>

In [examples/end_statistics_over_param_plot.py](./examples/end_statistics_over_param_plot_py.html), on the other hand, we apply the same method to analyze the impact of an algorithm parameter on the performance.
We again apply an [RLS algorithm](./moptipy.algorithms.so.html#module-moptipy.algorithms.so.rls) algorithm, but this time with a configurable operator, [Op1MoverNflip](./moptipy.operators.bitstrings.html#module-moptipy.operators.bitstrings.op1_m_over_n_flip), which flips each bit in a string with a probability distributed according to `Bin(m/n)`, where `n` is the total number of bits and `m` is a parameter.
We apply this algorithm for different values of `m` to two instances of the minimization version of the [LeadingOnes](./moptipy.examples.bitstrings.html#module-moptipy.examples.bitstrings.leadingones) problem.
We plot the mean end result after 128 FEs (on the vertical axis) over the values of `m` (horizontal axis).

<a href="./_static/mean_f_over_param.png">
<img alt="Example for the mean end result quality over the algorithm parameter m of the Bin(m/n) operator plugged into the RLS algorithm on several LeadingOnes instances." src="./_static/mean_f_over_param.png" style="width:70%;max-width:70%;min-width:70%" />
</a>

These plots have been implemented in the module [moptipy.evaluation.plot_end_statistics_over_parameter](./moptipy.evaluation.html#module-moptipy.evaluation.plot_end_statistics_over_parameter).


## 6.8. End Results Table

In the file [examples/end_results_table.py](./examples/end_results_table_py.html), you can find some code running a small experiment and creating an "end results table."
...
Tables can be rendered to different formats, such as [Markdown](./moptipy.utils.html#moptipy.utils.markdown.Markdown), [LaTeX](./moptipy.utils.html#moptipy.utils.latex.LaTeX), and [HTML](./moptipy.utils.html#moptipy.utils.html.HTML).
The example [examples/end_results_table.py](./examples/end_results_table_py.html), for instance, produces the following [Markdown](./moptipy.utils.html#moptipy.utils.markdown.Markdown) table:
...
It also produces the same table in [LaTeX](./moptipy.utils.html#moptipy.utils.latex.LaTeX):
...
The end result tables are implemented in the module [moptipy.evaluation.tabulate_end_results](./moptipy.evaluation.html#module-moptipy.evaluation.tabulate_end_results).


## 6.9. Testing End Results for Statistically Significant Differences (Table)

In the file [examples/end_results_tests.py](./examples/end_results_tests_py.html), you can find some code running a small experiment and creating a table of statistical end result tests.
...
The output in markdown of the table generated in [examples/end_results_tests.py](./examples/end_results_tests_py.html) looks as follows:
...
As you can see, we compare three algorithms, `rls_flip1`, `rls_flipB1`, and `rls_flipB2`, on six problem instances, the 100 and 200 bit versions of the  [`LeadingOnes`](./moptipy.examples.bitstrings.html#module-moptipy.examples.bitstrings.leadingones), [`OneMax`](./moptipy.examples.bitstrings.html#module-moptipy.examples.bitstrings.onemax), and [`Trap`](./moptipy.examples.bitstrings.html#module-moptipy.examples.bitstrings.trap), problems.
...
The end result comparison tables are implemented in the module [moptipy.evaluation.tabulate_result_tests](./moptipy.evaluation.html#module-moptipy.evaluation.tabulate_result_tests).
...
- [continuous_optimization.py](./examples/continuous_optimization_py.html) applies a set of numerical/continuous optimization algorithms to a simple problem and prints their results.
- [continuous_optimization_with_logging.py](./examples/continuous_optimization_with_logging_py.html) is exactly the same example, but this time log files are created and their contents are printed for each run.
- [ecdf_plot.py](./examples/ecdf_plot_py.html) runs a small experiment on the [`OneMax`](./moptipy.examples.bitstrings.html#module-moptipy.examples.bitstrings.onemax) problem and plots the [ECDF](#ecdf-plots).
- [end_results_jssp.py](./examples/end_results_jssp_py.html) runs a small experiment with on the Job Shop Scheduling Problem (JSSP) and generates an [end results CSV file](#end-result-csv-files).
- [end_results_plot.py](./examples/end_results_plot_py.html) applies two algorithms to the JSSP and creates [plots of end results](#end-results-plot).
- [end_results_table.py](./examples/end_results_table_py.html) runs another small experiment on the JSSP and generates a [table of end results](#end-results-table).
- [end_results_tests.py](./examples/end_results_tests_py.html) runs a small experiment on bit string search spaces and generates a [table with statistical comprisons of end results](#testing-end-results-for-statistically-significant-differences-table).
- [end_results_with_limits_plot.py](./examples/end_results_with_limits_plot_py.html) runs a small experiment with one algorithm on three [`OneMax`](./moptipy.examples.bitstrings.html#module-moptipy.examples.bitstrings.onemax) instances and collects the whole algorithm progress in the log files. It then plots [plots of end results](#end-results-plot) for different runtime limits.
- [end_statistics_jssp.py](./examples/end_statistics_jssp_py.html) runs a small experiment on the JSSP and generates an [end statistics CSV file](#end-result-statistics-csv-files).
- [end_statistics_over_feature_plot.py](./examples/end_statistics_over_feature_plot_py.html) solves several [`OneMax`](./moptipy.examples.bitstrings.html#module-moptipy.examples.bitstrings.onemax) instances and plots the ERT over the problem scale, i.e., generates a [performance-over-feature plot](#performance-over-algorithm-parameter-or-instance-feature).
- [end_statistics_over_param_plot.py](./examples/end_statistics_over_param_plot_py.html) applies different settings of an algorithm to LeadingOnes instances and plots their [performance over their parameter setting](#performance-over-algorithm-parameter-or-instance-feature).
- [ert_plot.py](./examples/ert_plot_py.html) applies an algorithm to the [`OneMax`](./moptipy.examples.bitstrings.html#module-moptipy.examples.bitstrings.onemax) and plots the [ERT](#expected-running-time-ert-plots) over the solution qualities.
- [ertecdf_plot.py](./examples/ertecdf_plot_py.html) applies one algorithm to several [`OneMax`](./moptipy.examples.bitstrings.html#module-moptipy.examples.bitstrings.onemax) instances and creates an [ERT-ECDF plot](#ert-ecdf-plots).
- [experiment_2_algorithms_4_problems.py](./examples/experiment_2_algorithms_4_problems_py.html) shows how to use the [structured experiment API](#how-to-run-a-series-of-experiments) and applies two algorithms to four problem instances ([`OneMax`](./moptipy.examples.bitstrings.html#module-moptipy.examples.bitstrings.onemax) and LeadingOnes).
- [experiment_own_algorithm_and_problem.py](./examples/experiment_own_algorithm_and_problem_py.html) shows how to [implement](#applying-an-own-algorithm-to-an-own-problem) some of the core components of our API, namely how a [self-implemented algorithm](#define-a-new-algorithm) can be applied to a [self-implemented problem](#define-a-new-problem-type).
- [log_file_jssp.py](./examples/log_file_jssp_py.html) showcases the [log file structure](#log-files) for single-objective optimization.
- [mo_example.py](./examples/mo_example_py.html) is a simple example for multi-objective optimization: we apply multi-objective RLS to a multi-objective version of the JSSP.
- [mo_example_nsga2.py](./examples/mo_example_nsga2_py.html) the same simple example for multi-objective optimization, but this time using the popular NSGA-II algorithm, which works out better than our multi-objective RLS.
- [mo_example_nsga2_bits.py](./examples/mo_example_nsga2_bits_py.html) another example of NSGA-II solving a multi-objective optimization problem, this time over the space of the bit strings.
- The package [`moptipy.examples.jssp`](./moptipy.examples.jssp_py.html) contains a complete experiment on the Job Shop Scheduling Problem (JSSP) together with its evaluation routines, making up an epxerimental part of the book ["Optimization Algorithms"](https://thomasweise.github.io/oa).
- [progress_plot.py](./examples/progress_plot_py.html) shows how [progress plots](#progress-plots) can be generated from a small experiment with the [`OneMax`](./moptipy.examples.bitstrings.html#module-moptipy.examples.bitstrings.onemax) problem and the 1-dimensional Ising model.
- [single_run_rls_onemax.py](./examples/single_run_rls_onemax_py.html) shows how we can perform a [single run of a single algorithm on a single problem instance](#how-to-apply-1-optimization-algorithm-once-to-1-problem-instance).
...
To ease such testing for you, we provide a set of tools for testing implemented algorithms, spaces, and operators in the package [moptipy.tests](./moptipy.tests.html).
Here, you can find functions where you pass in instances of your implemented components and they are checked for compliance with the [moptipy API](./moptipy.api.html).
...
We also try to extensively test our own code, see the [coverage report](./tc/index.html).

Examples for the variety of testing tools provided are:

- [`validate_component`](./moptipy.tests.html#moptipy.tests.component.validate_component) checks whether an object is a valid `moptipy` [component](./moptipy.api.html#module-moptipy.api.component) (which is the base class of all `moptipy` objects).
  It tests whether the conversion to string yields a valid name without invalid characters and whether [logging](./moptipy.api.html#moptipy.api.component.Component.log_parameters_to) of the component parameters works.
- [`validate_algorithm`](./moptipy.tests.html#moptipy.tests.algorithm.validate_algorithm) first checks if an [algorithm](./moptipy.api.html#moptipy.api.algorithm.Algorithm) is valid `moptipy` component and then applies it to a (user-provided) example problem.
...
- [`validate_mo_algorithm`](./moptipy.tests.html#moptipy.tests.mo_algorithm.validate_mo_algorithm) is the [multi-objective](./moptipy.api.html#moptipy.api.mo_algorithm.MOAlgorithm) version of `validate_algorithm`.
- [`validate_objective`](./moptipy.tests.html#moptipy.tests.objective.validate_objective) checks whether an [objective function](./moptipy.api.html#moptipy.api.objective.Objective) is implemented consistently, i.e., if its upper and lower bound are valid, if the result of evaluating some random solutions falls within these bounds, if it really only returns integers if it [claims to do so](./moptipy.api.html#moptipy.api.objective.Objective.is_always_integer), if it returns the same objective value for the same solution, and so on.
- [`validate_encoding`](./moptipy.tests.html#moptipy.tests.encoding.validate_encoding) checks whether an [encoding](./moptipy.api.html#module-moptipy.api.encoding) is implemented consistently.
- [`validate_op0`](./moptipy.tests.html#moptipy.tests.op0.validate_op0), [`validate_op1`](./moptipy.tests.html#moptipy.tests.op1.validate_op1), and [`validate_op2`](./moptipy.tests.html#moptipy.tests.op2.validate_op2) check whether [nullary](./moptipy.api.html#moptipy.api.operators.Op0), [unary](./moptipy.api.html#moptipy.api.operators.Op1), and [binary](./moptipy.api.html#moptipy.api.operators.Op2) operators are implemented consistently, respectively.
- [`validate_space`](./moptipy.tests.html#moptipy.tests.space.validate_space) checks whether an object is a consistent implementation of a `moptipy` [`Space`](./moptipy.api.html#moptipy.api.space.Space).
...
- [bit string](./moptipy.tests.html#module-moptipy.tests.on_bitstrings)-based problems,
- [permutation](./moptipy.tests.html#module-moptipy.tests.on_permutations)-based problems, or on
- the [JSSP](./moptipy.tests.html#module-moptipy.tests.on_jssp)
...
Besides the [basic `moptipy` dependencies](./requirements-dev_txt.html), it requires [a set of additional dependencies](./requirements-dev_txt.html).
...
Experiments with [`moptipy`](./) are reproducible and repeatable (according to the [ACM definition](https://www.acm.org/publications/policies/artifact-review-and-badging-current)) if the results are recorded in [log files](#log-files).
As stated in the [log files section](#log-files), our log files should store all the information relevant to a single run of an optimization algorithm.
First, the log files can store the complete [algorithm setups and objective function information](#the-section-setup) as well as the involved fully-qualified class names.
They also store the [system configuration](#the-section-sys_info), which includes the versions of the libraries used.
This should allow to re-create algorithm setups and system configuration.

Each run of the optimization algorithms on every problem instance is provided with a seeded [random number generator](https://numpy.org/doc/stable/reference/random/generator.html) via [`process.get_random()`](./moptipy.api.html#moptipy.api.process.Process.get_random).
...
The random seed using the generator as well as the [`numpy`](https://numpy.org/) classes of the generator and the `numpy` version are all stored in the [log files](#log-files).
The random seed for a new run can be set via the [`Execution`](./moptipy.api.html#moptipy.api.execution.Execution) builder object.
...
Additionally, if the [experiment API](./moptipy.api.html#module-moptipy.api.experiment) is used, then the random seeds are [determined based on the instance names](./moptipy.utils.html#moptipy.utils.nputils.rand_seeds_from_str).
...
This, in turn, means that the algorithms start with the same first random solutions (if they use the same [nullary operator](./moptipy.api.html#moptipy.api.operators.Op0)).
...
For executing experiments, the method [`run_experiment`](./moptipy.api.html#moptipy.api.experiment.run_experiment) from module [`moptipy.api.experiment`](./moptipy.api.html#module-moptipy.api.experiment) is used, as explained in [Section 3.2](#how-to-run-a-series-of-experiments).

It creates the log file and folder structure discussed in [Section 5.1.1.](#file-names-and-folder-structure) in a replicable way.
...
Under Linux, [`run_experiment`](./moptipy.api.html#moptipy.api.experiment.run_experiment) supports the parameter `n_threads`.
...
- The documentation of our project is available at <./>.
...
- [`cmaes`](https://github.com/CyberAgent/cmaes) provides implementations of Covariance Matrix Adaptation Evolution Strategies for solving continuous optimization problems.
  ...
  We wrap several of the algorithms into our `moptipy` API in module [`moptipy.algorithms.so.vector.cmaes_lib`](./moptipy.algorithms.so.vector.html#module-moptipy.algorithms.so.vector.cmaes_lib).
...
- The library "Powell's Derivative-Free Optimization solvers" ([`pdfo`](https://www.pdfo.net)) provides an implementation of the "Bound Optimization BY Quadratic Approximation" algorithm, or BOBYQA for short, for solving continuous optimization problems.
  ...
  We wrap its `BOBYQA` implementation into our `moptipy` API in module [`algorithms.so.vector.pdfo`](./moptipy.algorithms.so.vector.html#module-moptipy.algorithms.so.vector.pdfo).
...
- [`scipy`](https://scipy.org) offers a set of well-established mathematical optimization techniques for continuous optimization via the function [`scipy.optimize.minimize`](https://docs.scipy.org/doc/scipy/reference/generated/scipy.optimize.minimize.html).
  We wrap some of them into our `moptipy` API in module [`algorithms.so.vector.scipy`](./moptipy.algorithms.so.vector.html#module-moptipy.algorithms.so.vector.scipy).
...
1. The [IOHprofiler](https://iohprofiler.github.io) is a nice piece of open source software for analyzing the performance of optimization algorithms.
   It is possible to [convert](#export-to-iohanalyzer) our `moptipy` [log data](#file-names-and-folder-structure) to the format understood by the IOHanalyzer, which allows you to use this software to analyze your optimization results as well.
...
2. Thomas Weise. *Optimization Algorithms*. 2021-ongoing. <https://thomasweise.github.io/oa>.
   This is a book introducing metaheuristic optimization methods using `moptipy` as a source for example implementations and [showcase experiments](./moptipy.examples.jssp.html#module-moptipy.examples.jssp.experiment).
3. Tianyu Liang, Zhize Wu, Jörg Lässig, Daan van den Berg, Thomas Weise. Solving the Traveling Salesperson Problem using Frequency Fitness Assignment. *IEEE Symposium on Foundations of Computational Intelligence (IEEE FOCI'22)*, part of the *IEEE Symposium Series on Computational Intelligence (SSCI'22)*, December 4-7, 2022, Singapore. doi:[10.1109/SSCI51031.2022.10022296](https://doi.org/10.1109/SSCI51031.2022.10022296)
   This paper investigates Frequency Fitness Assignment ([FFA](./moptipy.algorithms.so.fitnesses.html#module-moptipy.algorithms.so.fitnesses.ffa)) on the Traveling Salesperson Problem.
...
Dr. Thomas Weise (see [Contact](#contact)) holds the copyright of this package *except* for the JSSP instance data in file [`moptipy/examples/jssp/instances.txt`](https://github.com/thomasWeise/moptipy/blob/main/moptipy/examples/jssp/instances.txt).

`moptipy` is provided to the public as open source software under the [GNU GENERAL PUBLIC LICENSE, Version 3, 29 June 2007](./LICENSE.html).
