...
from .objective import Objective, _CallableComponent
from .mapping import Mapping
from .space import Space
from ._process_no_ss import _ProcessNoSS
from ._process_ss import _ProcessSS
from ._process_no_ss_log import _ProcessNoSSLog
from ._process_ss_log import _ProcessSSLog
...
class Algorithm(Component):
    ...
    def solve(self, process: Process) -> None:
        ...
        :param Process process: the black-box process
        """
        raise NotImplementedError


class CallableAlgorithm(_CallableComponent, Algorithm):
    """An class for wrapping a Callable such as a lambda into an algorithm."""
    ...
    def __init__(self,
        ...
        :param Callable algorithm: the function to wrap, can be a lambda expression
...
def solve(algorithm: Algorithm,
    ...
    This means that an instance :class:`~moptipy.api.Process` will be constructed.
    Then the method :func:`~moptipy.api.Algorithm.solve` will be invoked.
    It may optionally log the progress and results to a file ``log_file``.
    Finally, the :class:`~moptipy.api.Process` instance will be returned.
    ...
    :param max_time_millis: the maximum runtime permitted, in milliseconds
    :param max_fes: the maximum objective function evaluations permitted
    ...
    :param bool overwrite_log: can the log file be overwritten if it exists?
    :return: the :class:`~moptipy.api.Process` instance which can be queried for the final result
    """

    if not (isinstance(algorithm, Algorithm)):
        raise ValueError("Optimization algorithm needs to be specified, but found '"
                         + str(type(algorithm)) + "'.")

    if search_space is None:
        if not (representation_mapping is None):
            raise ValueError("If search_space is not provided, neither can representation_mapping.")
    elif representation_mapping is None:
        if search_space == solution_space:
            search_space = None
        else:
            raise ValueError("If a search_space != solution_space is provided, "
                             "representation_mapping must be provided, too.")
