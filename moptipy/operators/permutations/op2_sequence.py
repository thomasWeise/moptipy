"""The sequence crossover operator."""
...
class Op2Sequence(Op2):
    ...
    def op2(self, random: Generator, dest: np.ndarray,
        ...
        desti: int = 0  # all array indexes = 0
        x0i: int = 0
        x1i: int = 0
        ...
        while True:  # repeat until dest is filled, i.e., desti=length
            ...
            op: int = x0[x0i] if ri(2) <= 0 else x1[x1i]
            dest[desti] = op  # store the operation in the destination
            desti = desti + 1  # step dest index
            if desti >= length:
