"""Test the onemax objective function."""
...
def _onemax(x: np.ndarray, dim: int) -> int:
    ...
    for xx in x:
        if xx:
            s += 1
    if (s < 0) or (s > dim):
        raise ValueError(f"x={x}, len(x)={len(x)}, dim={dim}, onemax(x)={s}!")
    return dim - s


def test_onemax() -> None:
    """Test the onemax objective function."""
    random: Final[Generator] = default_rng()
    space: BitStrings = BitStrings(int(random.integers(2, 32)))
    f: OneMax = OneMax(space.dimension)
    ...
    for x0 in [True, False]:
        ...
        for x1 in [True, False]:
            ...
            f2 = _onemax(x, 2)
            if f1 != f2:
                raise ValueError(f"onemax({x})={f1}, but {f1}!={f2}!")
