...
from moptipy.algorithms.so.ffa.ffa_hill_climber import FHC
...
def test_fhc_on_jssp() -> None:
    """Validate the FHC on the JSSP."""

    def create(instance: Instance, search_space: Permutations,
               objective: Objective) -> FHC:
        assert isinstance(instance, Instance)
        assert isinstance(search_space, Permutations)
        assert isinstance(objective, Objective)
        return FHC(Op0Shuffle(search_space), Op1Swap2(), False)

    validate_algorithm_on_jssp(create)


def test_fhc_on_onemax() -> None:
    """Validate the FHC on the OneMax problem."""

    def create(bs: BitStrings, objective: Objective) -> FHC:
        assert isinstance(bs, BitStrings)
        assert isinstance(objective, Objective)
        return FHC(Op0Random(), Op1MoverNflip(bs.dimension, 1, True), False)

    validate_algorithm_on_onemax(create)


def test_fhc_on_onemax_with_large_range() -> None:
    """Validate the FHC on the OneMax problem."""
    ...
    def create(bs: BitStrings, objective: Objective) -> FHC:
        ...
        return FHC(Op0Random(), Op1MoverNflip(bs.dimension, 1, True), True)

    validate_algorithm_on_onemax(create)


def test_fhc_on_leadingones() -> None:
    """Validate the FHC on the LeadingOnes problem."""
...
def test_fhc_on_leadingones_large_range() -> None:
    """Validate the FHC on the LeadingOnes problem with larger bounds."""
    ...
    def create(bs: BitStrings, objective: Objective) -> FHC:
        ...
        objective.lower_bound = __lb  # type: ignore
        objective.upper_bound = __ub  # type: ignore
        return FHC(Op0Random(), Op1MoverNflip(bs.dimension, 1, True))
...
def test_h_log() -> None:
    ...
    space = BitStrings(n)
    problem = OneMax(n)
    algorithm = FHC(Op0Random(), Op1Flip1(), True)
