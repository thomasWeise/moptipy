...
from moptipy.algorithms.so.ffa.safea_n import SAFEAN
...
def test_safean_on_jssp() -> None:
    """Validate the SAFEA-N on the JSSP."""

    def create(instance: Instance, search_space: Permutations,
               objective: Objective) -> SAFEAN:
        assert isinstance(instance, Instance)
        assert isinstance(search_space, Permutations)
        assert isinstance(objective, Objective)
        return SAFEAN(Op0Shuffle(search_space), Op1Swap2(),
                      ExponentialSchedule(10.0, 1e-10), False)

    validate_algorithm_on_jssp(create)


def test_safean_on_onemax() -> None:
    """Validate the SAFEA-N on the OneMax problem."""

    def create(bs: BitStrings, objective: Objective) -> SAFEAN:
        assert isinstance(bs, BitStrings)
        assert isinstance(objective, Objective)
        return SAFEAN(Op0Random(), Op1MoverNflip(bs.dimension, 1, True),
                      ExponentialSchedule(10.0, 1e-10), False)

    validate_algorithm_on_onemax(create)


def test_safean_on_onemax_with_large_range() -> None:
    """Validate the SAFEA-N on the OneMax problem."""
    ...
    def create(bs: BitStrings, objective: Objective) -> SAFEAN:
        ...
        return SAFEAN(Op0Random(), Op1MoverNflip(bs.dimension, 1, True),
                      ExponentialSchedule(10.0, 1e-10), True)

    validate_algorithm_on_onemax(create)


def test_safean_on_leadingones() -> None:
    """Validate the SAFEA-N on the LeadingOnes problem."""
...
def test_safean_on_leadingones_large_range() -> None:
    """Validate the SAFEA-N on the LeadingOnes problem with larger bounds."""
    ...
    def create(bs: BitStrings, objective: Objective) -> SAFEAN:
        ...
        objective.upper_bound = __ub  # type: ignore
        return SAFEAN(Op0Random(), Op1MoverNflip(bs.dimension, 1, True),
                      ExponentialSchedule(10.0, 1e-10))
...
def test_h_log() -> None:
    ...
    space = BitStrings(n)
    problem = OneMax(n)
    algorithm = SAFEAN(Op0Random(), Op1Flip1(),
                       ExponentialSchedule(10.0, 1e-10), True)
