...
from moptipy.algorithms.so.ffa.safea_a import SAFEAA
...
def test_safeaa_on_jssp() -> None:
    """Validate the SAFEA-A on the JSSP."""

    def create(instance: Instance, search_space: Permutations,
               objective: Objective) -> SAFEAA:
        assert isinstance(instance, Instance)
        assert isinstance(search_space, Permutations)
        assert isinstance(objective, Objective)
        return SAFEAA(Op0Shuffle(search_space), Op1Swap2(),
                      ExponentialSchedule(10.0, 1e-10), False)

    validate_algorithm_on_jssp(create)


def test_safeaa_on_onemax() -> None:
    """Validate the SAFEA-A on the OneMax problem."""

    def create(bs: BitStrings, objective: Objective) -> SAFEAA:
        assert isinstance(bs, BitStrings)
        assert isinstance(objective, Objective)
        return SAFEAA(Op0Random(), Op1MoverNflip(bs.dimension, 1, True),
                      ExponentialSchedule(10.0, 1e-10), False)

    validate_algorithm_on_onemax(create)


def test_safeaa_on_onemax_with_large_range() -> None:
    """Validate the SAFEA-A on the OneMax problem."""
...
def test_safeaa_on_leadingones() -> None:
    """Validate the SAFEA-A on the LeadingOnes problem."""
...
def test_safeaa_on_leadingones_large_range() -> None:
    """Validate the SAFEA-A on the LeadingOnes problem with larger bounds."""
...
def test_h_log() -> None:
    ...
    space = BitStrings(n)
    problem = OneMax(n)
    algorithm = SAFEAA(Op0Random(), Op1Flip1(),
                       ExponentialSchedule(10.0, 1e-10), True)
