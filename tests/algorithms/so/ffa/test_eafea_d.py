...
from moptipy.algorithms.so.ffa.eafea_d import EAFEAD
...
def test_eafeac_on_jssp() -> None:
    """Validate the EAFEA-D on the JSSP."""

    def create(instance: Instance, search_space: Permutations,
               objective: Objective) -> EAFEAD:
        assert isinstance(instance, Instance)
        assert isinstance(search_space, Permutations)
        assert isinstance(objective, Objective)
        return EAFEAD(Op0Shuffle(search_space), Op1Swap2(), False)

    validate_algorithm_on_jssp(create)


def test_eafeac_on_onemax() -> None:
    """Validate the EAFEA-D on the OneMax problem."""

    def create(bs: BitStrings, objective: Objective) -> EAFEAD:
        assert isinstance(bs, BitStrings)
        assert isinstance(objective, Objective)
        return EAFEAD(Op0Random(), Op1MoverNflip(bs.dimension, 1, True),
                      False)

    validate_algorithm_on_onemax(create)


def test_eafeac_on_onemax_with_large_range() -> None:
    """Validate the EAFEA-D on the OneMax problem."""
    ...
    def create(bs: BitStrings, objective: Objective) -> EAFEAD:
        ...
        return EAFEAD(Op0Random(), Op1MoverNflip(bs.dimension, 1, True), True)

    validate_algorithm_on_onemax(create)


def test_eafeac_on_leadingones() -> None:
    """Validate the EAFEA-D on the LeadingOnes problem."""
...
def test_eafeac_on_leadingones_large_range() -> None:
    """Validate the EAFEA-D on the LeadingOnes problem with larger bounds."""
...
def test_h_log() -> None:
    ...
    space = BitStrings(n)
    problem = OneMax(n)
    algorithm = EAFEAD(Op0Random(), Op1Flip1(), True)
